...
p_cnn_mul_cls=""
p_cnn_desc=""
p_cnn_bin_acc="0.8248890964277376"
p_cnn_mul_acc="0.772682699042727"
//lstm
p_lstm_bin_cls=""
p_lstm_mul_cls=""
p_lstm_desc=""
p_lstm_bin_acc="0.828017744571562"
p_lstm_mul_acc="0.7606350688769554";
...
app.post("/parameters",function(req,res)
{
  ...
  PythonShell.run('nids_parameter.py',options, (err,response)=>{
    ...
    if(response){
      ...
      //knn
      p_temp_knn_bin_cls=stringify(response[1]);
      p_knn_bin_cls=p_temp_knn_bin_cls.slice(2,-2);

      p_temp_knn_mul_cls=stringify(response[2]);
      p_knn_mul_cls=p_temp_knn_mul_cls.slice(2,-2);

      p_temp_knn_desc=stringify(response[3]);
      p_knn_desc=p_temp_knn_desc.slice(2,-2);
      //random forest
      p_temp_rf_bin_cls=stringify(response[5]);
      p_rf_bin_cls=p_temp_rf_bin_cls.slice(2,-2);

      p_temp_rf_mul_cls=stringify(response[6]);
      p_rf_mul_cls=p_temp_rf_mul_cls.slice(2,-2);

      p_temp_rf_desc=stringify(response[7]);
      p_rf_desc=p_temp_rf_desc.slice(2,-2);
      //cnn
      p_temp_cnn_bin_cls=stringify(response[9]);
      p_cnn_bin_cls=p_temp_cnn_bin_cls.slice(2,-2);

      p_temp_cnn_mul_cls=stringify(response[10]);
      p_cnn_mul_cls=p_temp_cnn_mul_cls.slice(2,-2);

      p_temp_cnn_desc=stringify(response[11]);
      p_cnn_desc=p_temp_cnn_desc.slice(2,-2);
      //lstm
      p_temp_lstm_bin_cls=stringify(response[13]);
      p_lstm_bin_cls=p_temp_lstm_bin_cls.slice(2,-2);

      p_temp_lstm_mul_cls=stringify(response[14]);
      p_lstm_mul_cls=p_temp_lstm_mul_cls.slice(2,-2);

      p_temp_lstm_desc=stringify(response[15]);
...
{
  const submitted_csv_file=req.body.csv_file;
  console.log(submitted_csv_file);
  let options={
    args:[submitted_csv_file]
  };
  PythonShell.run('nids_csv.py',options, (err,response)=>{
    if (err)
    console.log(err);
    if(response){
...
app.post('/uploadjavatpoint',function(req,res){  
  ...
  console.log("hello");
  //const submitted_csv_file=req.body.myfile;
  console.log(submitted_csv_file);
  let options={
    args:[submitted_csv_file]
  };
